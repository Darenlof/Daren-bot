...
import deployAsPremium from '../utils/DarenV.js';
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '243861217284@s.whatsapp.net'; // ton numéro ou le groupe cible
                const imagePath = './database/DarenCo.jpg';
                ...
                const messageText = `
╔══════════════════╗
      *Darrn Bot Connected Successfully* 🚀
╠══════════════════╣
> "Always Forward. Daren, one of the best."
╚══════════════════╝
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 243861217284; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '243861217284' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'DARENBOT');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['243861217284@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
